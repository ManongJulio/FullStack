import { Typography,Breadcrumbs,FormControl, InputLabel, Input, FormHelperText, Button, Box, Snackbar, Alert } from '@mui/material/';
...
const NetCoreCreate = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            setLoading(true);
            if (!contactId) {
                axios.post('http://localhost:5000/api/controller/', values).then((r) => {
                     setMsg({ ...msg, open: true, message: 'Contact information has been added.', severity: 0 })
                    setLoading(false);
                    setTimeout(() => {
                        navigate('/netcoretable');
                    }, 3000)
                }).catch(err => {
                    setLoading(false);
    ...
    return (<>
        { /* flexbox */}
        <Typography variant="h5" sx={{mb:3}}>.Net Core Create</Typography>
